...
namespace _20180628_SP.v1
{
    public partial class FrmSenadores : Form
    {
        ...
        public void ManejadorVoto(string senador, Votacion.EVoto voto)
        {
            ...
            {
                //Votacion.Simular recall = new Votacion.NOMBRE_EVENTO(this.ManejadorVoto);
            ...
            else
            {
                ...
                if (aux == 0)
                {
                    ...
                    try
                    {
                        serializarXML.Guardar(Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "\\votacion.xml", this.votacion);
                    }
                    catch(Exception)
                    {
